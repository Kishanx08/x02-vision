...
class MediaProcessor:
    ...
    def __init__(
        ...
        self.text_moderation_service = text_moderation_service or TextModerationService()
    ...
    def get_file_type(self, file_path: str) -> str:
        ...
        if ext == '.gif':
            ...
            raise UnsupportedMediaTypeError(f"Unsupported file format: {ext or 'missing extension'}")
    ...
    def download_file(self, url: str) -> str:
        ...
        try:
            ...
            # Save to temp file
            temp_path = os.path.join(self.temp_dir, f"x02_download_{os.getpid()}_{hash(url) % 10000}{ext}")
    ...
    def extract_video_frames(
        ...
        try:
            ...
            if duration > 3600:  # 1 hour
                logger.warning("Video is longer than 1 hour, processing with larger interval")
            ...
            while True:
                ...
                if frame_idx % effective_interval == 0:
                    ...
                    if extracted_count >= max_frames:
                        logger.warning(f"Reached max frame limit ({max_frames}), stopping extraction")
                        break
    ...
    def process_frames(self, frames: List[np.ndarray]) -> Dict:
        ...
        for batch_start in range(0, len(frames), self.batch_size):
            try:
                ...
                logger.error(f"Error processing frames {batch_start}-{batch_start + self.batch_size - 1}: {e}")
                continue
    ...
    def analyze_text_content(self, frames: List[np.ndarray]) -> Dict:
        """Run OCR and text moderation on sampled frames."""
        if not self.enable_ocr:
            return {"enabled": False, "available": False, "reason": "OCR disabled"}
        ocr_result = self.ocr_service.extract_text(frames)
        text_result = self.text_moderation_service.moderate_ocr_result(ocr_result)
        ocr_result["matched"] = text_result.get("matched", False)
        ocr_result["categories"] = text_result.get("categories", [])
        ocr_result["recommendation"] = text_result.get("recommendation", "allow")
        ocr_result["text_moderation"] = text_result
        return ocr_result
    
    def make_decision(self, average_score: float, max_score: float, flagged_percentage: float) -> str:
        """
    ...
    def process_media(
        ...
        try:
            ...
            if not frame_results['frame_scores']:
                raise FrameInferenceFailedError('Frame inference failed for all extracted frames')
            ...
            ocr_analysis = self.analyze_text_content(frames) if ocr_requested else {
                "enabled": False,
                "available": False,
            ...
            if ocr_analysis.get("recommendation") == "soft_flag" and recommendation == "allow":
