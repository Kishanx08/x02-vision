# x02 Unified Media Moderation API v2.1
...
It uses:
- [efficientnet_model.py](C:\Users\Kishan Soni\Downloads\media_moderation\efficientnet_model.py) for EfficientNet-B4 inference
- [media_processor.py](C:\Users\Kishan Soni\Downloads\media_moderation\media_processor.py) for download, extraction, and aggregation
- [media_moderation_api.py](C:\Users\Kishan Soni\Downloads\media_moderation\media_moderation_api.py) for the API layer
- [services/ocr_service.py](C:\Users\Kishan Soni\Downloads\media_moderation\services\ocr_service.py) for OCR backends
- [services/text_moderation_service.py](C:\Users\Kishan Soni\Downloads\media_moderation\services\text_moderation_service.py) for DistilBERT-based text moderation
- [services/job_queue.py](C:\Users\Kishan Soni\Downloads\media_moderation\services\job_queue.py) for async GIF/video jobs
...
```bash
python media_moderation_api.py
```

Recommended production command behind Nginx:

```bash
uvicorn media_moderation_api:app --host 127.0.0.1 --port 8000 --workers 1
```
...
- See [DEPLOY_UBUNTU.md](C:\Users\Kishan Soni\Downloads\media_moderation\DEPLOY_UBUNTU.md) for the full VPS setup.
...
That temporary file is deleted in the `finally` block in [media_processor.py](C:\Users\Kishan Soni\Downloads\media_moderation\media_processor.py#L503), so it is cleaned up after processing completes or if processing raises after the download step.

Uploaded files are also deleted after each request in [media_moderation_api.py](C:\Users\Kishan Soni\Downloads\media_moderation\media_moderation_api.py).
